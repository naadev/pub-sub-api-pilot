...
def encode(schema, payload):
    ...
    return buf.getvalue()
def makePublishRequest(schemaid,userId,fieldValues,eventId):
    dt = datetime.now() + timedelta(days=5)
    payload = {
        "CreatedDate": int(datetime.now().timestamp()),
        "CreatedById": userId, #Your user ID        
    }
    for k in fieldValues.keys():
        payload[k] = fieldValues[k]
...
with open(certifi.where(), 'rb') as f:
    ...
    with grpc.secure_channel('eventbusapi-core1.sfdc-ypmv18.svc.sfdcfc.net:7443', creds) as channel:
        ...
        print(stub)
        mypubtopic = "/event/Order_Event__e"
        ...
        while goLoop:
            print('*'*10)
            Order_Number = input("what is the order number (text - 10) ?")
            Has_Shipped = input ("Has Shipped value (True/False) ?")=='True' 
            eventId = int(input("event Id (int (4)?" ))
            publishresponse = stub.Publish(pb2.PublishRequest(topic_name=mypubtopic, events=
            makePublishRequest(schemaid,myUserId,{"Order_Number__c": str(Order_Number),
                        "Has_Shipped__c": Has_Shipped},str(eventId) )),
                                                 metadata=authmetadata)
            pp.pprint(publishresponse)
            print('*'*10)                                            
            goLoop = input('Do you want to send an other message (True/False) ?')=='True' 
